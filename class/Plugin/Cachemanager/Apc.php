 ...
 *  @package    Ethna
 */
class Ethna_Plugin_Cachemanager_Apc extends Ethna_Plugin_Cachemanager
{
    /**#@+  @access private */

    /** @var    object  Apc    Apc オブジェクト */
    private $m = null;
    ...
    public function get($key, $lifetime = null, $namespace = null)
    {
        ...
        if (!$value) {
            return Ethna::raiseWarning(
                sprintf('No such cache, key="%s"', $key), E_CACHE_NO_VALUE
            );
        }
        ...
        if ($lifetime !== null) {
            if (($time + $lifetime) < time()) {
                return Ethna::raiseWarning('Cache lifetime expired', E_CACHE_EXPIRED);
            }
        }
    ...
    public function set($key, $value, $timestamp = null, $namespace = null, $expiration = null)
    {
        ...
        if (!apc_store($cache_key, array('time' => $time, 'data' => $value), $expiration)) {
            return Ethna::raiseError(
                sprintf('Failed to set cache, key="%s"'), E_CACHE_GENERAL
            );
        }
     ...
     */
    public function setApcOption($opt, $value)
    {
     ...
     */
    public function getApcOption($opt)
    {
